...
def get_process_privileges(pid):
  try:
    hproc = win32api.OpenProcess(win32con.PROCESS_QUE_INFORMATION, False, pid)
    ...
    for priv_id, flags in privs:
      if flags == (win32security.SE_PRIVILEGE_ENABLED |
        ...
        privilege = win32security.LookupPrivilegeName(None, priv_id)
        privileges += f'{privilege}|'
...
def monitor():
  ...
  while True:
    try:
      ...
      process_log_message = (
        f'{cmdline}, {create_data}, {executable},'
